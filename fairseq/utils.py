...
def clip_grad_norm_(params, max_norm, aggregate_norm_fn=None) -> torch.Tensor:
    if isinstance(params, torch.Tensor):
        params = [params]
    params = list(params)
    grads = [p.grad.detach() for p in params if p is not None and p.grad is not None and not hasattr(p, 'expert')]
    split_grads = [p.grad.detach() for p in params if p is not None and p.grad is not None and  hasattr(p, 'expert')]
    if len(grads) == 0:
        if len(params) > 0:
    ...
    if len(split_grads) > 0:
        split_norm = torch.norm(torch.stack([torch.norm(g) for g in split_grads]))
        if dist.is_initialized():
            split_norm.pow_(2)
            dist.all_reduce(split_norm)
            split_norm.sqrt_()
        total_norm = torch.norm(torch.stack([split_norm, total_norm]))
