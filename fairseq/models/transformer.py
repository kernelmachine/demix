...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--desynchronize', default=False, action='store_true',
        ...
        parser.add_argument('--moe-top1-expert', default=False, action='store_true',
                            help="Use top1 gate instead of top2")
        parser.add_argument('--add-expert-layers', default=False, action='store_true',
                            help="Use top1 gate instead of top2")
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        for i in range(args.encoder_layers):
            is_moe_layer = args.moe_freq != 0 and (i + 1) % args.moe_freq == 0
    ...
    def forward(
        ...
        for layer in self.layers:
            x, l_aux_i = layer(x, encoder_padding_mask)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        if self.decoder_layerdrop > 0.0:
            self.expert_layers = LayerDropModuleList(p=self.decoder_layerdrop)
        else:
        ...
        for i in range(args.decoder_layers):
            ...
            self.layers.append(
                self.build_decoder_layer(
                    ...
                    add_expert_ffn=args.add_expert_ffn
                )
            )
        self.num_layers = len(self.layers)
        if args.add_expert_layers:
            for i in range(args.decoder_layers):
                is_moe_layer = args.moe_freq != 0 and (i + 1) % args.moe_freq == 0
            ...
            self.num_expert_layers = len(self.expert_layers)
    ...
    def build_decoder_layer(
            self, args, no_encoder_attn=False, is_moe_layer=False, add_expert_ffn=False):
        layer = TransformerDecoderLayer(
            args, no_encoder_attn=no_encoder_attn, is_moe_layer=is_moe_layer, add_expert_ffn=add_expert_ffn
        )
        if getattr(args, "checkpoint_activations", False):
            offload_to_cpu = getattr(args, "offload_activations", False)
            layer = checkpoint_wrapper(layer, offload_to_cpu=offload_to_cpu)
    ...
    def forward(
        ...
        # if not features_only:
        # #     # if self.decoders and len(self.decoders) > 1:
        # #     #     x = self.moe_output_layer(x)
        # #     # else:
        ...
        if self.decoders is not None:
            for decoder in self.decoders:
                ...
                additional_features.append(add_x.unsqueeze(0))

            mats = [x.unsqueeze(0)] + additional_features 

            mats = torch.cat(mats, 0)


                

            # for mat in mats[1:]:
            #     x = torch.dot(self.adapter, x + mat)
            #     self.adapter 
    ...
    def extract_features_scriptable(
        ...
        l_aux = []
        if self.args.add_expert_layers:
            layers = enumerate(zip(self.layers, self.expert_layers))
        else:
            layers = enumerate(zip(self.layers, [None] * self.num_layers))
        for idx, (shared_layer, expert_layer) in layers:
            if incremental_state is None and not full_context_alignment:
                self_attn_mask = self.buffered_future_mask(x)
            else:
                self_attn_mask = None

            x_shared, layer_attn, _, l_aux_i = shared_layer(
                x,
            ...
            if expert_layer is not None:
                x_expert, _, _, l_aux_i = expert_layer(
                    x,
                    encoder_out["encoder_out"][0]
                    if (encoder_out is not None and len(encoder_out["encoder_out"]) > 0)
                    else None,
            ...
            if layer_attn is not None and idx == alignment_layer:
                attn = layer_attn.float().to(x_shared)
    ...
    def add_adapter(self):
        self.adapter = Adapter(self.embed_dim)
    ...
    def add_decoders(self, decoders):
        ...
        # gate = Top1Gate(self.embed_dim, len(self.decoders)+1)

        # gate.wg = gate.wg.to(self.layers[0].self_attn.k_proj.weight.device)
        # experts = torch.nn.ModuleList([x.decoder.output_projection for x in decoders] + [self.output_projection])
        # self.moe_output_layer = MOELayer(gate, experts, self.args)
        
        # self.adapters = torch.nn.ModuleList()
        # for _ in range(1):
        #     z = Adapter(50264)
        #     self.adapters.append(Adapter(50264))
        
        # self.adapter.weight = torch.nn.Parameter(self.output_projection.weight)
        # for decoder in self.decoders:
        #     self.adapter.weight = torch.nn.Parameter(self.adapter.weight + decoder.decoder.output_projection.weight)
        # self.adapter.weight = torch.nn.Parameter(self.adapter.weight / (len(self.decoders) + 1))
        # self.output_projection = None
        # for decoder in self.decoders:
        #     decoder.decoder.output_projection = None
    ...
    def output_layer(self, features):
        ...
        if self.adaptive_softmax is None:
            ...
            # # if self.moe_output_layer is not None:
            # #     features = self.moe_output_layer(features)
            # #     return features
            # # else:
            # else:
            output_proj = self.output_projection(features)
            return output_proj
        else:
            # if self.adapter is not None:
            #     # for adapter in self.adapters:
...
def base_architecture(args):
    ...
    args.add_expert_ffn = getattr(args, "add_expert_ffn", True)
