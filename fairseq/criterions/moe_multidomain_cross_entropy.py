...
@register_criterion("moe_multidomain_cross_entropy", dataclass=MoECrossEntropyCriterionConfig)
class MoECrossEntropyCriterion(FairseqCriterion):
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        losses = [log.get("loss", 0) for log in logging_outputs]
        ntokens_ = [log.get("ntokens", 0) for log in logging_outputs]
        sample_sizes = [log.get("sample_size", 0) for log in logging_outputs]
        for idx in range(len(losses)):
            if sample_size != ntokens_[idx]:
                metrics.log_scalar(
                    f"nll_loss_{idx}", losses[idx] / ntokens_[idx] / math.log(2), ntokens_[idx], round=3
                )
                metrics.log_scalar(
                    f"ppl_{idx}", 2 ** (losses[idx] / ntokens_[idx] / math.log(2)),  round=3
                )
            else:
                metrics.log_derived(
                    f"ppl_{idx}", 2 ** (losses[idx] / samples_sizes[idx] / math.log(2)), round=3
                )
